  ...
  else
   {
    for (int i = 0; i < scanResults; ++i) {
      ...
      if (SSID.indexOf("Node") == 0) {
        // SSID of interest
        Serial.print(i + 1); Serial.print(": "); Serial.print(SSID); Serial.print(" ["); Serial.print(BSSIDstr); Serial.print("]"); Serial.print(" ("); Serial.print(RSSI); Serial.print(")"); Serial.println("");
        // Get BSSID => Mac Address of the Edge
        int mac[6];
...
// Check if the edge is already paired with the fog.
// If not, pair the edge with the fog
void manageNode() {
  if (nodeCount > 0) {
    ...
    // No Edge found to process
    Serial.println("No Edge found to process");
  }
}
...
// callback when data is sent from FOG to Edge
...
void loop() {
  ScanForNode();
  // If Edge is found, it would be populate in `Edge` variable
  // We will check if `Edge` is defined and then we proceed further
  if (nodeCount > 0) { // check if Edge channel is defined
    // `Edge` is defined
    // Add Edge as peer if it has not been added already
    manageNode();
