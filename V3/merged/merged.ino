...
void NotifyFog()
{
  WiFi.mode(WIFI_STA);
    esp_now_peer_info_t peerInfo = {};
    memcpy(&peerInfo.peer_addr, fogAddr, 6);
    if (!esp_now_is_peer_exist(fogAddr))
    {
      esp_now_add_peer(&peerInfo);
    }
    // prepare message
...
float acc_x, acc_y,acc_z,acceleration,speed2;
void Printv2()
{
...
void accident_detection()
{
  ...
  // Detect accident if the change in acceleration exceeds a threshold
  float threshold = 3; // have to adjust this value
  if (abs(acceleration - accelerationPrev) > threshold) {
    // Handle and notify 
