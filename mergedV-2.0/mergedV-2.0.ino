...
void CalculateDistanceAndPredictSpeed()
{
  ...
  double predicted_cover_dist = (/*[GPS Speed self car in km/hr]*/ gps.speed.kmph() * 2 * (1000 / 3600)) + (0.5 * accelerationPrev * pow(2, 2)); // #s=u*t+0.5*a*pow(t,2)
  double front_car_predicted_cover_dist = (/*[GPS Speed front min distance car in km/hr]*/ gpsDataArray[index - 1].speed * 2 * (1000 / 3600)) + (0.5 * accelerationPrev * pow(2, 2));
...
float calculateAcceleration(float timeInterval)
{
  if (gps.location.isValid() == 1)
  {
    ...
    accelerationPrev = (currentSpeed - previousSpeed) / timeInterval;
    previousSpeed = currentSpeed;
  }
  Serial.println("Aclleartion :" + String(accelerationPrev));
  return accelerationPrev;
}
...
void broadGps()
{
  ...
  for (int i = 0; i < nodeCount; i++)
  {
    ...
    gpsDataSend.accelaration = accelerationPrev;
...
float acc_x, acc_y,acc_z,acceleration,speed2, tempacc, diff;
void Printv2()
{
...
display.setCursor(60,20);
  display.print("S: ");
display.println(String(speed2, 5));
display.setTextSize(1);
...
void UpdateMPUdata()
{
 ...
 float  gx =gyr.x;
  ...
  //  acceleration = sqrt(acc_x*acc_x+ acc_y*acc_y);
  //  diff = acceleration - tempacc ;
  ...
  if (time_diff >= 1)
  {
   ...
   acceleration = sqrt(acc_x*acc_x+ acc_y*acc_y) * 9.81;
   ...
   Serial.println("Previous Acceleration (m/s^2): " + String(tempacc, 6));
   Serial.println("Current Acceleration (m/s^2): " + String(acceleration, 6));
   Serial.println("Differnece  (m/s^2): " + String(diff, 6));
  ...
  if (diff <=0)
  {
    ...
    if (diff==0)
    {
     ...
     Serial.println("Negative Condition Executed" );
  speed -= diff * time_diff;
  speed2=speed;
  }
  else 
  {
...
void setup()
{
  ...
  gpsData.accelaration = accelerationPrev;
...
void loop()
{
  ...
  Serial.println("Accelaeration " + String(accelerationPrev));
